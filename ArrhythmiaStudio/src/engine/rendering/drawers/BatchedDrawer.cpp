...
BatchedDrawer::BatchedDrawer()
{
	begin = batchedMeshes.size();
	end = batchedMeshes.size();
}

void BatchedDrawer::appendMesh(Mesh* mesh, glm::mat4 transform, glm::vec4 color)
{
	batchedMeshes.emplace_back(mesh, transform, color);
	end++;
}
...
void BatchedDrawer::draw(glm::mat4 cameraMatrix)
{
	...
	{
		BatchMeshInfo& info = batchedMeshes[i];

		Mesh& mesh = *info.mesh;
		glm::mat4& transform = info.transform;
...
void BatchedDrawer::clearBatches()
{
	batchedMeshes.clear();
}
